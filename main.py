...
def get_database_config():
    """الحصول على إعدادات قاعدة البيانات من البيئة أو استخدام محلي"""
    try:
        # إذا كان هناك رابط قاعدة بيانات سحابية (لـ Render)
        if 'DATABASE_URL' in os.environ:
            ...
            # استخدام قاعدة البيانات المحلية (للتنمية)
            return {
                'driver': '{SQL Server}',
...
async def get_referral(update: Update, context: CallbackContext) -> int:
    """معالجة كود الإحالة المدخل من المستخدم"""
    try:
        referral_input = update.message.text.strip()
        
        if referral_input.lower() in ['لا', 'no', 'skip', 'لأ']:
            ...
            if await validate_referral_code(referral_input):
                context.user_data['invited_by'] = referral_input
                
                await update.message.reply_text(
                    f"✅ تم التحقق من كود الدعوة: {referral_input}\n\n"
                    "🆔 **الآن، ما هو اسمك الثلاثي الكامل؟**\n"
                    "(مثال: أحمد محمد علي)"
                )
                ...
                await update.message.reply_text(
                    "❌ كود الدعوة غير صحيح!\n"
                    "الرجاء إدخال كود صحيح أو اكتب 'لا' للمتابعة."
                )
                return REFERRAL_STAGE
...
def export_user_data():
    """تصدير بيانات المستخدمين المهمة"""
    try:
        if CONNECTION_STRING:
            ...
            for row in cursor.fetchall():
                users_data.append({
                    ...
                    'registration_date': str(row[7]) if row[7] else None
                })
            
            # تصدير إحصائيات التعليقات
            cursor.execute('''
            ...
            logger.info(f"✅ تم إنشاء نسخة احتياطية: {len(users_data)} مستخدم, {comment_stats['completed_tasks']} مهمة مكتملة")
